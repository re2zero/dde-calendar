...
bool CSchedulesDBus::GetJobs(quint32 startYear, quint32 startMonth, quint32 startDay, quint32 endYear, quint32 endMonth, quint32 endDay, QVector<ScheduleDateRangeInfo> &out)
{
    QList<QVariant> argumentList;
    ...
    if (reply.type() != QDBusMessage::ReplyMessage) {
        qDebug() << reply.signature() << reply.type() << reply.errorName() << reply.errorMessage();
        return false;
    }
...
bool CSchedulesDBus::QueryJobs(QString key, QDateTime starttime, QDateTime endtime, QString &out)
{
    ...
    QList<QVariant> argumentList;
    strJson = "End\":\"2021-05-18T13:34:15+08:00\",\"Key\":\"新建\",\"Start\":\"2020-05-18T13:34:15+08:00\"";
    argumentList << QVariant::fromValue(strJson);
    QDBusMessage reply = callWithArgumentList(QDBus::Block, QStringLiteral("QueryJobs"), argumentList);
