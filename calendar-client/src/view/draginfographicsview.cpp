...
void DragInfoGraphicsView::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if (event->mimeData()->hasFormat("Info")) {
        ...
        if ((event->source() != this && info->recurs()) || !isCanDragge(info)) {
            ...
            qCDebug(ClientLogger) << "Accepting drag for schedule:" << info->summary();
            event->accept();
            //设置被修改的日程原始信息
...
{
    qCDebug(ClientLogger) << "DragInfoGraphicsView::setPressSelectInfo with info:" << info->summary();
...
{
    qCDebug(ClientLogger) << "DragInfoGraphicsView::updateScheduleInfo with info:" << info->summary();
    QVariant variant;
    //获取主窗口指针
...
{
    // qCDebug(ClientLogger) << "DragInfoGraphicsView::DeleteItem with info:" << info->summary();
    if (info.isNull()) {
...
{
    // qCDebug(ClientLogger) << "DragInfoGraphicsView::isCanDragge with info:" << info->summary();
