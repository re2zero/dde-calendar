...
{
    // qCDebug(ClientLogger) << "CAllDayEventWeekView::MoveInfoProcess called for schedule:" << info->summary();
    Q_UNUSED(pos);
    if (info->allDay()) {
...
{
    qCDebug(ClientLogger) << "CAllDayEventWeekView::setSelectSearchSchedule called for schedule:" << info->summary();
...
void CAllDayEventWeekView::upDateInfoShow(const DragStatus &status, const DSchedule::Ptr &info)
{
    ...
    case ChangeBegin:
    case ChangeEnd: {
        qCDebug(ClientLogger) << "Status: ChangeBegin/ChangeEnd for schedule:" << info->summary();
